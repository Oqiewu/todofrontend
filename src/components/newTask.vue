...
import cap from "./capComponent.vue"
...
export default {
  ...
  methods: {
    render_tasks_list({data}){
      ...
      let tasks_class = document.querySelector(".tasks_block")
      let cap = document.querySelector(".cap")
      if (this.$store.state.label_id == 0){
        this.tasks = []
        tasks_class.style.display = "none"
        cap.style.display = ""
      } else {
        tasks_class.style.display = ""
    ...
    create_new_task() {
      let newTask = {
        text: this.text,
        label_id: this.$store.state.label_id
      };
      this.text = ""
    ...
    get_tasks_list() {
      if (this.$store.state.label_id == 0) {
        this.tasks = []
        this.render_tasks_list({data: []})
        return
      }
      axios.get("http://127.0.0.1:8000/tasks?label_id="+ this.$store.state.label_id).then(this.render_tasks_list)
    }
  },
  mounted() {
    this.get_tasks_list();
  ...
  created() {
    this.$root.$on("label_changed", this.get_tasks_list)
    this.$root.$on("label_deleted", this.get_tasks_list)
  }
};
</script>
